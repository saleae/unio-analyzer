...
:	Analyzer2(),
	...
	mInputSpikeSuppressionTimeS( 150E-9 )
{
...
void UnioAnalyzer::AdvanceChannelDataEdgesToSample( U64 sample )
{
...
void UnioAnalyzer::GetTransaction()
{
	...
	else if( header_result == HeaderInvalidResetPulseFound )
	{
		return;
	}
	...
	for( ; ; )
	{
		...
		//at least partial overlap. potentially fixable.
		if( frame.mStartingSampleInclusive < mLastEndingSample )
		{
			//start location of frame went backwards. not fixable.
			if( frame.mStartingSampleInclusive < mLastStartingSample )
				AnalyzerHelpers::Assert( "" );

			//frame fully inclusive. not fixable.
			if( frame.mEndingSampleInclusive < mLastEndingSample )
				AnalyzerHelpers::Assert( "" );

			//HACK
			frame.mStartingSampleInclusive = mLastEndingSample + 1;
		...
		mLastEndingSample = frame.mEndingSampleInclusive;
		mLastStartingSample = frame.mStartingSampleInclusive;
...
UnioHeaderResult UnioAnalyzer::GetHeader()
{
	...
	U64 last_edge_sample = start_of_sync;
	
	for( U32 i = 0; i < 9; i++ )
	{
		mScio->AdvanceToNextEdge();
		if( mScio->GetBitState() == BIT_LOW && ( ( mScio->GetSampleNumber() - last_edge_sample ) >= mStandyPulseSamples ) )
		{
			//we encountered a reset pulse in this stage.
			return HeaderInvalidResetPulseFound; //we're currently located on the falling edge of a reset pulse. hmm..
		}
		last_edge_sample = mScio->GetSampleNumber();
	}
...
UnioBitResult UnioAnalyzer::GetBit( bool allow_mak_hold )
{
	...
	AdvanceChannelDataEdgesToSample( quarter_way );

	//edges in the this region are kindof don't cares.  

	U64 three_quarters_way = mBitStartingSample + mSamplesPerThreeQuartersBit;

	U32 num_edges = mScio->GetNumberOfTransitionsToAbsPositionMinimumWidth( three_quarters_way, mInputSpikeSuppressionSamples + 1 );
	...
	if( num_edges == 0 )
	{
		...
		}else
		{
			...
			AdvanceChannelDataEdgesToSample( three_quarters_way );
			mScio->AdvanceToNextEdge();
